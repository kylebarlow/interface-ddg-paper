...
We set out to develop and assess methods for estimating experimentally determined changes in binding free energy after mutation (interface \ddg) within the Rosetta macromolecular modeling suite. Rosetta is freely available for academic use, and allows combination of these interface \ddg\ predictions with Rosetta's powerful protein design capabilities, which have proven successful in a variety of applications \cite{mandell_computer-aided_2009,kaufmann_practically_2010}.
...
Inclusion of recent Rosetta energy function and sampling method developments, including methods that attempt to more aggressively sample conformational space, has not resulted in significant improvement to the alanine scanning method.

We sought to create a method that would take into account aspects of the conformational plasticity of proteins by representing structures as an ensemble of individual full-atom models that would effectively explore biologically relevant and accessible portions of conformational space near the native folds of proteins.
...
When compared to ensembles generated via molecular dynamics simulations or the ``PertMin'' method\cite{davey_improving_2014}, backrub ensembles were shown to be the only ensembles capable of generating higher diversity (as measured with RMSD) between output models than from output models to the original input crystal structure.
...
We used SAbDab \cite{dunbar_sabdab:_2014} to defined complexes that contained at least one antibody binding partner.
...
While the flex ddG method shows improved performance on the subset of multiple mutations as compared to the control and ddg_monomer methods, flex ddg did not match the performance of the ZEMu method on this subset.
...
While the results presented above used an ensemble size of 50 members, we desired to investigate what the ideal ensemble size would be to maximize the predictive ability of our method.
...
As the sampling and scoring problems of protein modeling are generally linked, it is often the case that improving one enables further improvements on the other.
...
In particular, performance with Rosetta's newest REF energy function\cite{alford_rosetta_2017} does not improve performance in our method when compared with the prior Talaris\cite{leaver-fay_chapter_2013,song_structure-guided_2011,shapovalov_smoothed_2011} energy function (\cref{tab:table-ref}), indicating that the backrub sampling parameters might require further benchmarking and adaption to the REF energy function.
